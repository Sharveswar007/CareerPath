...
export function useProctoring({ onViolation, maxViolations = 3, enabled = true }: UseProctoringOptions) {
    ...
    useEffect(() => {
        ...
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            // Never interfere with standard alphabetic keys or typing anywhere
            const key = e.key;
            if (key === "a" || key === "s" || key === "d" || key === "A" || key === "S" || key === "D") {
                return;
            }
            if (key.length === 1 && !e.ctrlKey && !e.metaKey && !e.altKey) {
                return; // Standard character typing — never block
            }

            // CRITICAL: Never interfere with Monaco Editor or text inputs
            const target = e.target as HTMLElement;
            if (
                target &&
                (
            ...
            // Only block specific dangerous system shortcuts on non-editor elements
        ...
        console.log("[PROCTORING v10] Listeners attached — bulletproof typing guard (bypasses all typing & editor inputs)");
